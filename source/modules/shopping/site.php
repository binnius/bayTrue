...
class ShoppingModuleSite extends WeModuleSite {
	...
	public function doMobileConfirm() {
		...
		if (checksubmit('submit')) {
			...
			message('提交订单成功，现在跳转至付款页面...', 'https:/mobile.paypal.com/cgi-bin/webscr?business=baydumpling@gmail.com&cmd=_xclick&currency_code=USD&amount=1&item_name=Orderid', 'success');
		}
		$carttotal = 0;
		foreach ($cart['goods'] as $row) {
			$carttotal += $row['total'];
